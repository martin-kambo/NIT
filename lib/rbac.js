...
// Thin wrapper around hasPermission() for route handlers that need a
// scope check on data only available once the request body/params/query
// have been parsed (so it can't be expressed as route-level Express
// middleware the way requireRole/requireAnyRole/requireMinRole can). The
// authorization DECISION still lives entirely in hasPermission() above --
// this only adapts its boolean return into the same "check, auto-respond,
// tell the caller whether to continue" shape every route handler already
// uses.
//
// Phase 4B.5: moved verbatim from server.js (was a bare module-scoped
// function there, calling RBAC.hasPermission() across the module
// boundary). Here it calls the local hasPermission() directly since both
// now live in the same file. Behavior, signature, and the exact 403 body
// are unchanged.
function requirePermission(req, res, opts) {
  if (hasPermission(req, opts)) return true;
